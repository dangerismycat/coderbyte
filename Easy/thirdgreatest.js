/*	Have the function ThirdGreatest(strArr) take the array of strings stored in
	strArr and return the third largest word within in. So for example: if strArr is
...
function ThirdGreatest(strArr) { 
  ...
  temp = strArr.sort(function(a, b) {
    return a.length > b.length
  });

  if (temp[temp.length - 3].length === temp[temp.length - 2].length) {
    if (temp[temp.length - 2].length === temp[temp.length - 1].length) {
